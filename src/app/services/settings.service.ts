...
export class SettingsService {
  ...
  create(setting: ISetting) {
    ...
    return this.httpClient.post<ISetting>(url, setting).pipe(shareReplay({ bufferSize: 1, refCount: true }))
  }

  read() {
    var url = this.baseUrl + '/v1/read_setting';
    return this.httpClient.get<ISetting[]>(url).pipe(shareReplay({ bufferSize: 1, refCount: true }))
  }
